...
public class UdpVideoThread extends Thread {

    private static final String TAG = UdpVideoThread.class.getSimpleName();
    ...
    public UdpVideoThread(SurfaceView surfaceView) {
    ...
    public void run() {
        try {
            Log.d(TAG, "run 111");
            initializeDecoder();
            datagramSocket = new DatagramSocket(Constant.UDP_VIDEO_RECEIVE_PORT);
            while (isRunning) {
                Log.d(TAG, "prepare receive video data");
                byte[] container = new byte[15000];
                datagramPacket = new DatagramPacket(container, container.length);
                datagramSocket.receive(datagramPacket);
                byte[] data = datagramPacket.getData();
                int len = datagramPacket.getLength();
                Log.d(TAG, "receive data len : " + len);
                feedDataToDecoder(data);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
